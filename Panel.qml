...
Panel {
  ...
  // Actively re-probe on a short interval rather than relying on NM's own
  // schedule. NM only rechecks on device activation or certain internal
  // triggers, then caches the result indefinitely -- observed sitting on a
  // stale "full" for 20+ minutes through a PIA disconnect/reconnect that a
  // plain `nmcli networking connectivity` (no `check`) never surfaced,
  // because that reads the cache. Only `nmcli networking connectivity check`
  // forces a fresh probe, which is what checkConnectivity() below does.
  Timer {
    id: connectivityPoll
    interval: root.restricted ? 10000 : 20000
    repeat: true
    running: root.connectivityChecksEnabled
